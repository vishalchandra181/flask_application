...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:root@localhost:5432/testdb1234'
...
def user_dashboard():
    ...
    for url_detail in url_details3:
        ...
        try:
            # response = requests.post(url, auth=HTTPBasicAuth(username, password),data={ 'client_id': "controller", "grant_type": "password"})
            response = requests.post(url,
                                     data={'client_id': "controller", "grant_type": "password", "username": username,
                                           "password": password})
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        if user.username == username and user.password == password and user.is_admin == False:
            flash('Change default admin password', 'success')
        ...
        if action == 'removeurl':
            try:
                ...
                if len(user_ids) == 0:
                    ...
                    flash('Selected Url removed successfully.', 'success')
            except Exception:
                return redirect(url_for('error_page'))
    ...
    if request.method == 'POST':
        ...
        if action == 'removeown':
            for user_id in user_ids:
                ...
                db.session.commit()
            if len(user_ids) == 0:
                flash('Please! Select First...')
            else:
                flash('Selected credentials removed successfully.', 'success')
    return redirect(url_for('user_dashboard'))


@app.route('/adminlogin/admindashboard/registernewuser')
@login_required
def Register_new_user():
    normal_users = User.query.filter(User.id != 1).all()
    return render_template('Registed_user.html', normal_users=normal_users)
